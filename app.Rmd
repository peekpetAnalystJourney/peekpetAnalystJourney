...
price$Base_Price1 = as.numeric(price$Base_Price1) 
  ...
  dplyr::group_by(Year) %>% 
  dplyr::summarise(
    Base_Price = base::sum(Base_Price2) /12,
    Low_Price = base::sum(Low_Price2) / 12,
    High_Price = base::sum(High_Price2)/12
  ) 
...
css_pth = "#__next > div:nth-child(2) > div:nth-child(4) > section > div > div:nth-child(1) > div > div:nth-child(1) > table > tbody > tr:nth-child(1) > td:nth-child(2)"
fx <- rvest::read_html(fx_url)
fx <- fx %>% rvest::html_element(css = css_pth) %>% rvest::html_text2()
fx <- str_split(fx," ",n=6,simplify = TRUE)[1]
fx <- as.numeric(fx)
```
...
**Project ** with the highest NPV and is therefore selected first. 
