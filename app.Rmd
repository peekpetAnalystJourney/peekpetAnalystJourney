...
Please download the Rmarkdown file [from this link]()
...
raw_prod %>% 
  ...
  ggplot(aes(x=Year,y=value,fill=series))+geom_area()+
  labs(title = "Annual Oil Production Volume bbl",y="oil volume bbl")

raw_prod %>% 
  pivot_longer(-Year, names_to = "series",values_to = "value") %>% 
  group_by(series) %>% 
  summarise("Reserve MMbbl"=sum(value)/1e6) %>% 
  knitr::kable(caption = "Project Reserve",digits=2)
```

2. WCS price is derived from WTI future price ([source](https://www.oilcrudeprice.com/wti-oil-price-future/)) and a constant WCS differential of USD18/bbl. Price only available till 2031, set as fixed value till 2039.
...
price <- as.data.frame(df) %>% 
  ...
  tidyr::separate(Contract,into=c("month","Year")) %>% 
    ...
    dplyr::rename(Base_Price = Last) %>%
  dplyr::mutate(Base_Price = Base_Price - wcs_diff)%>% 
  dplyr::mutate(Low_Price = Base_Price*0.5,High_Price = Base_Price*1.5) %>% 
  dplyr::filter(Year>2022&Year<2032) %>% 
  group_by(Year) %>% 
  summarise(Base_Price=mean(Base_Price),Low_Price=mean(Low_Price),High_Price=mean(High_Price)) 

fixed_base = 39
fixed_high = 39*1.5
fixed_low = 39*0.5

  price <- rbind(price,c(2032,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2033,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2034,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2035,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2036,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2037,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2038,fixed_base,fixed_low,fixed_high))
  price <- rbind(price,c(2039,fixed_base,fixed_low,fixed_high))
...
4. Fx via Web scrapping from [xe.com](https://www.xe.com/currencyconverter/convert/?Amount=1&From=USD&To=CAD)
...
df2 %>% 
  ...
  annotate("text",x="Project_1",y=df2$Cost[1]/2,angle=90, label=sprintf("CA$%.0f",df2$Cost[1]),color='white',size=7) +
  annotate("text",x="Project_2",y=df2$Cost[2]/2,angle=90, label=sprintf("CA$%.0f",df2$Cost[2]),color='white',size=7) +
  annotate("text",x="Project_3",y=df2$Cost[3]/2,angle=90, label=sprintf("CA$%.0f",df2$Cost[3]),color='white',size=7) +
  annotate("text",x="Project_4",y=df2$Cost[4]/2,angle=90, label=sprintf("CA$%.0f",df2$Cost[4]),color='white',size=7)
```
...
df4a <- tibble(Category=c("NPV (CA$ million)","IRR","Productivity Index"), 
       "Project 1"=c(FinancialMath::NPV(cf0=df2$Cost[1],cf=df3a$Project1_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[1],cf=df3a$Project1_Cashflow_AT,times=seq(1,length(df3a$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[1],cf=df3a$Project1_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/df2$Cost[1]),
       
       "Project 2"=c(FinancialMath::NPV(cf0=df2$Cost[2],cf=df3a$Project2_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[2],cf=df3a$Project2_Cashflow_AT,times=seq(1,length(df3a$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[2],cf=df3a$Project2_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/df2$Cost[2]),
       
       "Project 3"=c(FinancialMath::NPV(cf0=df2$Cost[3],cf=df3a$Project3_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[3],cf=df3a$Project3_Cashflow_AT,times=seq(1,length(df3a$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[3],cf=df3a$Project3_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/df2$Cost[3]),
       
       "Project 4"=c(FinancialMath::NPV(cf0=df2$Cost[4],cf=df3a$Project4_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[4],cf=df3a$Project4_Cashflow_AT,times=seq(1,length(df3a$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[4],cf=df3a$Project4_Cashflow_AT,times=seq(1,length(df3a$Year),1),i=disc_factor)/df2$Cost[4])
...
df4b <- tibble(Category=c("NPV (CA$ million)","IRR","Productivity Index"), 
       "Project 1"=c(FinancialMath::NPV(cf0=df2$Cost[1],cf=df3b$Project1_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[1],cf=df3b$Project1_Cashflow_AT,times=seq(1,length(df3b$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[1],cf=df3b$Project1_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/df2$Cost[1]),
       
       "Project 2"=c(FinancialMath::NPV(cf0=df2$Cost[2],cf=df3b$Project2_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[2],cf=df3b$Project2_Cashflow_AT,times=seq(1,length(df3b$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[2],cf=df3b$Project2_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/df2$Cost[2]),
       
       "Project 3"=c(FinancialMath::NPV(cf0=df2$Cost[3],cf=df3b$Project3_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[3],cf=df3b$Project3_Cashflow_AT,times=seq(1,length(df3b$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[3],cf=df3b$Project3_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/df2$Cost[3]),
       
       "Project 4"=c(FinancialMath::NPV(cf0=df2$Cost[4],cf=df3b$Project4_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/1e6,
                     FinancialMath::IRR(cf0=df2$Cost[4],cf=df3b$Project4_Cashflow_AT,times=seq(1,length(df3b$Year),1))[2],
                     1+FinancialMath::NPV(cf0=df2$Cost[4],cf=df3b$Project4_Cashflow_AT,times=seq(1,length(df3b$Year),1),i=disc_factor)/df2$Cost[4])
...
##### Economics results for Based Oil Price at different Discount Factor
...
server <- function(input, output) {
  ...
  tibble(Category=c("NPV (CA$ million)","IRR","Productivity Index"), 
...
##### Economics results at different Discount Factor and High Oil Price
...
##### Economics results at different Discount Factor and Low Oil Price
